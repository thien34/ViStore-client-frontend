...
export default function CategoryCard({ title, description, image, link, className }: CategoryCardProps) {
    return (
        <Link href={link}>
            <Card className={cn('flex h-full flex-row justify-between sm:flex-col', className)}>
                <CardHeader className='w-full p-4'>
                    <CardTitle className='text-base font-medium md:text-lg'>{title}</CardTitle>
                    <CardDescription className='text-pretty text-sm md:text-base'>{description}</CardDescription>
                </CardHeader>
