...
const Folder = forwardRef<
    ...
    FolderProps &
        React.HTMLAttributes<HTMLDivElement> & {
            IconOpen?: React.ReactNode // Custom IconOpen
            IconClose?: React.ReactNode // Custom IconClose
...
>(
    (
        ...
        IconOpen = IconOpen ? IconOpen : <FolderOpenIcon className='size-4' />
        IconClose = IconClose ? IconClose : <FolderIcon className='size-4' />
        return (
            <AccordionPrimitive.Item {...props} value={value} className='relative overflow-hidden h-full'>
                ...
                >
                    {expandedItems?.includes(value) ? (openIcon ?? IconOpen) : (closeIcon ?? IconClose)}
                    <span>{element}</span>
                </AccordionPrimitive.Trigger>
                <AccordionPrimitive.Content className='text-sm data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down relative overflow-hidden h-full'>
                    {element && indicator && <TreeIndicator aria-hidden='true' className={cn(indicatorClassName)} />}
                    <AccordionPrimitive.Root
                        dir={direction}
                        type='multiple'
